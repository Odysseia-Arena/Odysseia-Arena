...
from src.rating import glicko2_impl as glicko2
from src.utils.config import DATA_DIR, GLICKO2_DEFAULT_RATING, GLICKO2_DEFAULT_RD, GLICKO2_DEFAULT_VOL, GLICKO2_TAU
...
def get_completed_battles() -> List[Dict]:
    ...
    try:
        with sqlite3.connect(DATABASE_FILE) as conn:
            ...
            cursor = conn.execute(
                "SELECT timestamp, model_a_id, model_b_id, winner FROM battles WHERE status = 'completed' AND winner != 'skip' ORDER BY timestamp ASC"
            )
