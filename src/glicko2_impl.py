...
# Default values, these will be overridden by our config
MU = 1500
PHI = 350
SIGMA = 0.06
TAU = 1.0
...
class Glicko2(object):
    ...
    def rate(self, rating, series):
        ...
        difference = 0
        if not series:
            # ODYSSEIA-ARENA MODIFICATION:
            # For AI models with fixed strength, rating deviation should not
            # increase over time. We disable Step 6 for players who did not compete.
            phi_star = rating.phi
        ...
        # ODYSSEIA-ARENA MODIFICATION:
        # We disable the _preRatingRD() step (Step 6) for AI models with fixed strength.
        # In the original algorithm, this would be:
        # phi_star = math.sqrt(rating.phi ** 2 + sigma ** 2)
        # Here, we keep phi the same before the final update.
        phi_star = rating.phi
